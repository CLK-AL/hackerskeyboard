...
class InputLanguageSelection : PreferenceActivity() {
    ...
    private class Loc(var label: String, var locale: Locale) : Comparable<Any> {
        override fun toString(): String = label

        override fun compareTo(other: Any): Int {
            return sCollator.compare(label, (other as Loc).label)
        }
    ...
    override fun onPause() {
        ...
        val finalLanguages = if (checkedLanguages.length < 1) null else checkedLanguages
        val sp = PreferenceManager.getDefaultSharedPreferences(this)
        val editor = sp.edit()
        editor.putString(LatinIME.PREF_SELECTED_LANGUAGES, finalLanguages)
    ...
    private fun getUniqueLocales(): ArrayList<Loc> {
        ...
        for (kl in KBD_LOCALIZATIONS) {
            var localeCode = kl
            if (localeCode.length == 2 && langSet.contains(localeCode)) continue
            // replace zz_rYY with zz_YY
            if (localeCode.length == 6) localeCode = localeCode.substring(0, 2) + "_" + localeCode.substring(4, 6)
        ...
        for (i in 0 until origSize) {
            ...
            if (len == 2 || len == 5 || len == 6) {
                ...
                if (finalSize == 0) {
                    preprocess[finalSize++] = Loc(LanguageSwitcher.toTitleCase(l.getDisplayName(l)), l)
                } else {
    ...
    private fun arrayContains(array: Array<String>, value: String): Boolean {
        for (element in array) {
            if (element.equals(value, ignoreCase = true)) return true
        }
        return false
    }
    ...
    companion object {
        ...
        @JvmField
        val NOCAPS_LANGUAGES = HashSet<String>().apply {
            add("ar")
            add("iw")
            add("th")
        }
        ...
        @JvmField
        val NODEADKEY_LANGUAGES = HashSet<String>().apply {
            add("ar")
            add("iw") // TODO: currently no niqqud in the keymap?
            add("th")
        }

        // Languages which should not auto-add space after completions
        @JvmField
        val NOAUTOSPACE_LANGUAGES = HashSet<String>().apply {
            add("th")
        }
