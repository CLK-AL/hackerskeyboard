...
class LatinIMESettings : PreferenceActivity(),
    ...
    override fun onCreate(icicle: Bundle?) {
        ...
        mQuickFixes = findPreference(QUICK_FIXES_KEY) as CheckBoxPreference?
        ...
        mKeyboardModePortraitPreference = findPreference("pref_keyboard_mode_portrait") as ListPreference?
        mKeyboardModeLandscapePreference = findPreference("pref_keyboard_mode_landscape") as ListPreference?
    ...
    override fun onResume() {
        super.onResume()
        val autoTextSize = AutoText.getSize(listView)
        if (autoTextSize < 1) {
            (findPreference(PREDICTION_SETTINGS_KEY) as PreferenceGroup?)
        ...
        if (!LatinIME.sKeyboardSettings.compactModeEnabled) {
            ...
            if (oldEntries != null && oldValues != null && oldEntries.size > 2) {
    ...
    private fun updateSummaries() {
        val res = resources
        val settingsValue = mSettingsKeyPreference?.value
        val index = mSettingsKeyPreference?.findIndexOfValue(settingsValue) ?: 0
        mSettingsKeyPreference?.summary = res.getStringArray(R.array.settings_key_modes)[index]

        mInputConnectionInfo?.summary = String.format(
            "%s type=%s",
    ...
    private fun updateVoiceModeSummary() {
        val voiceValue = mVoicePreference?.value
        val index = mVoicePreference?.findIndexOfValue(voiceValue) ?: 0
        mVoicePreference?.summary = resources.getStringArray(R.array.voice_input_modes_summary)[index]
    }
    ...
    companion object {
        ...
        @JvmField
        val INPUT_CLASSES = HashMap<Int, String>().apply {
            put(0x00000004, "DATETIME")
            put(0x00000002, "NUMBER")
            put(0x00000003, "PHONE")
            put(0x00000001, "TEXT")
            put(0x00000000, "NULL")
        }

        @JvmField
        val DATETIME_VARIATIONS = HashMap<Int, String>().apply {
            put(0x00000010, "DATE")
            put(0x00000020, "TIME")
        }

        @JvmField
        val TEXT_VARIATIONS = HashMap<Int, String>().apply {
            put(0x00000020, "EMAIL_ADDRESS")
            put(0x00000030, "EMAIL_SUBJECT")
            put(0x000000b0, "FILTER")
            put(0x00000050, "LONG_MESSAGE")
            put(0x00000080, "PASSWORD")
            put(0x00000060, "PERSON_NAME")
            put(0x000000c0, "PHONETIC")
            put(0x00000070, "POSTAL_ADDRESS")
            put(0x00000040, "SHORT_MESSAGE")
            put(0x00000010, "URI")
            put(0x00000090, "VISIBLE_PASSWORD")
            put(0x000000a0, "WEB_EDIT_TEXT")
            put(0x000000d0, "WEB_EMAIL_ADDRESS")
            put(0x000000e0, "WEB_PASSWORD")
        }

        @JvmField
        val NUMBER_VARIATIONS = HashMap<Int, String>().apply {
            put(0x00000010, "PASSWORD")
        }
        ...
        private fun inputTypeDesc(type: Int): String {
            ...
            val variation = type and 0x00000ff0 // MASK_VARIATION
            ...
            when (cls) {
                InputType.TYPE_CLASS_TEXT -> {
                    val varName = TEXT_VARIATIONS[variation]
                    if (varName != null) {
                        out.append(".")
                        out.append(varName)
                    }
                ...
                InputType.TYPE_CLASS_NUMBER -> {
                    val varName = NUMBER_VARIATIONS[variation]
                    if (varName != null) {
                        out.append(".")
                        out.append(varName)
                    }
                ...
                InputType.TYPE_CLASS_DATETIME -> {
                    val varName = DATETIME_VARIATIONS[variation]
                    if (varName != null) {
                        out.append(".")
                        out.append(varName)
                    }
