...
class DeadAccentSequence(user: ComposeSequencing) : ComposeSequence(user) {
        ...
        if (composed != null) {
            if (composed == "") {
                ...
                if (Character.getType(c) != Character.NON_SPACING_MARK.toInt()) {
                    ...
                    val normalizedComposed = doNormalise(buildComposed.reverse().toString())
                    if (normalizedComposed == "") {
                        return true
                    }
                    clear()
                    composeUser?.onText(normalizedComposed)
                    return false
                } else {
                    return true
                }
    ...
    companion object {
        ...
        private fun putAccent(nonSpacing: String, spacing: String, ascii: String?) {
            val actualAscii = ascii ?: spacing
            put("$nonSpacing ", actualAscii)
